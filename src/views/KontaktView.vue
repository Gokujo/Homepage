    ...
    <section class="bg-white/90 rounded-lg p-6 mt-6">
      ...
      <form class="mt-6 space-y-4" @submit.prevent="submit">
        <div>
          ...
          <input id="name" v-model.trim="form.name" required class="w-full border rounded px-3 py-2" :placeholder="$t('contact.namePlaceholder')" />
        </div>
        <div>
          <label class="block text-sm font-medium mb-1" for="email">{{ $t('contact.email') }}</label>
          <input id="email" type="email" v-model.trim="form.email" required class="w-full border rounded px-3 py-2" placeholder="name@example.com" />
        </div>
        <div>
          <label class="block text-sm font-medium mb-1" for="message">{{ $t('contact.message') }}</label>
          <textarea id="message" v-model.trim="form.message" required rows="6" class="w-full border rounded px-3 py-2" :placeholder="$t('contact.messagePlaceholder')"></textarea>
        </div>
        <div>
          <label class="block text-sm font-medium mb-1">{{ $t('contact.captchaTitle') }}</label>
          <div class="flex items-center gap-2">
            <span class="text-sm">{{ captcha.a }} + {{ captcha.b }} =</span>
            <input type="number" v-model.number="captcha.answer" required class="w-24 border rounded px-3 py-2" :aria-label="$t('contact.captchaAria')" />
            <button type="button" class="text-blue-600 hover:underline text-sm" @click="regenerateCaptcha">{{ $t('contact.reload') }}</button>
          </div>
          <p v-if="captchaError" class="text-sm text-red-600 mt-1">{{ $t('contact.captchaError') }}</p>
        </div>
        ...
        <div>
          ...
          <div class="mt-1 flex items-center gap-2">
            <button type="button" class="text-blue-600 hover:underline text-sm" @click="resetTurnstile">{{ $t('contact.reload') }}</button>
            <span v-if="turnstileError" class="text-sm text-red-600">{{ $t('contact.turnstileError') }}</span>
          </div>
        </div>
        ...
        <div class="flex items-start gap-2">
          ...
          <label for="prices" class="text-sm">
            ...
            <RouterLink :to="{ name: 'dienstleistung', params: { locale: localeParam } }" class="text-blue-600 hover:underline">{{ $t('contact.pricesAgree.link') }}</RouterLink>
        ...
        <div class="flex items-center gap-2">
          ...
          <label for="privacy" class="text-sm">
            ...
            <RouterLink :to="{ name: 'datenschutz', params: { locale: localeParam } }" class="text-blue-600 hover:underline">{{ $t('contact.privacyAgree.link') }}</RouterLink>
            {{ $t('contact.privacyAgree.after') }}
          </label>
        </div>
        <div class="pt-2">
          <button type="submit" class="bg-blue-600 text-white px-4 py-2 rounded hover:bg-blue-700" :disabled="pending">
...
const form = ref({ name: '', email: '', message: '', prices: false, privacy: false })
const captcha = reactive({ a: 0, b: 0, answer: null })
const captchaError = ref(false)
const turnstileToken = ref('')
const turnstileWidgetId = ref(null)
const turnstileError = ref(false)
const sitekey = import.meta.env.VITE_CF_TURNSTILE_SITEKEY || '1x00000000000000000000AA'
const { t } = useI18n()
  ...
  const tryRender = () => {
    if (typeof window !== 'undefined' && window.turnstile && document.getElementById('cf-turnstile')) {
      turnstileWidgetId.value = window.turnstile.render('#cf-turnstile', {
        sitekey,
        callback: (token) => { turnstileToken.value = token; turnstileError.value = false },
        'error-callback': () => { turnstileToken.value = ''; turnstileError.value = true },
      })
      clearInterval(timer)
    }
  }
