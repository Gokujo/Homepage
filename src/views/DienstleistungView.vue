<template>
  <div class="max-w-5xl px-3 sm:px-4 mx-auto lg:mx-0 lg:pl-8 xl:pl-16" id="berechnung">
    <section class="bg-white/95 rounded-2xl p-6 mt-6 shadow" id="info">
      <span class="inline-block bg-green-600 text-white text-xs px-2 py-1 rounded">{{ $t('services.badgeTransparent') }}</span>
      ...
      <div class="mt-4 text-gray-800">
        ...
        <span class="bg-blue-600 text-white text-xs px-2 py-1 rounded">{{ $t('services.orientationBadge') }}</span>
      </div>
        ...
        <table class="min-w-full divide-y divide-gray-200">
            ...
            <tbody>
              <tr>
                ...
                <td class="py-2 px-4"><span id="stundensatz">{{ stundensatz }}</span> €</td>
              </tr>
              <tr>
                <td class="py-2 px-4 align-top">
                  <div class="font-semibold">{{ $t('services.rows.additionalCosts.title') }}</div>
                  <div class="text-sm text-gray-600">{{ $t('services.rows.additionalCosts.desc') }}</div>
                </td>
                <td class="py-2 px-4">
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-4 sm:gap-6 mt-6">
      <section class="bg-white/95 rounded-2xl p-6 lg:col-span-2 shadow" id="leistungen">
        <span class="inline-block bg-blue-600 text-white text-xs px-2 py-1 rounded">{{ $t('services.title') }}</span>
        <h2 class="text-2xl font-semibold mt-2">{{ $t('services.title') }}</h2>
        <small class="text-gray-600">{{ $t('services.subtitle') }}</small>
        <div class="mt-4 divide-y divide-gray-200">
          <div class="py-3 flex items-start justify-between gap-4" v-for="leistung in leistungen" :key="leistung.id">
            <div>
              <div class="font-semibold">{{ $t('services.items.' + leistung.id + '.name') }}</div>
              <div class="text-sm text-gray-600">{{ $t('services.items.' + leistung.id + '.desc') }}</div>
              <div class="mt-1 text-sm"><b>{{ $t('services.flat') }}:</b> {{ leistung.pauschal }} € | <b>{{ $t('services.hourly') }}:</b> {{ leistung.stunde }} €</div>
            </div>
            <button class="shrink-0 bg-blue-600 text-white px-3 py-1.5 rounded hover:bg-blue-700" @click="addToBasket(leistung)">{{ $t('services.addToOffer') }}</button>
          </div>
        </div>
      </section>
          ...
          <div v-if="basket.length" class="mt-4 space-y-2 text-sm sm:text-base">
            <div class="flex items-start justify-between gap-3" v-for="it in basket" :key="it.id">
              <div class="flex-1">
                ...
                <template v-else>
                  {{ it.name }} — {{ Number(it.pauschal).toFixed(2) }} €
                </template>
              </div>
               <button class="text-red-600 hover:text-red-700 text-pointer" @click="removeFromBasket(it.id)">{{ $t('services.remove') }}</button>
            </div>
            <div class="font-semibold pt-2">{{ $t('services.subtotalTitle') }}: {{ subtotal.toFixed(2) }} €</div>
            <div v-if="preisModus === 'stunde'" class="text-xs text-gray-500">{{ $t('services.minHoursNote') }}</div>
            <div class="pt-2">
              <RouterLink :to="{ name: 'kontakt', params: { locale: localeParam } }" class="inline-block bg-blue-600 text-white px-3 py-1.5 rounded hover:bg-blue-700">{{ $t('services.requestOffer') }}</RouterLink>
            </div>
          </div>
        </div>
      </section>
    ...
    <section class="bg-white/95 rounded-2xl p-6 mt-6 shadow" id="rechner">
      ...
      <div class="mt-4 grid grid-cols-1 sm:grid-cols-2 gap-4">
        <div v-if="calcMode === 'stunde'">
          ...
          <input type="number" min="8" class="w-full border rounded px-3 py-2" v-model.number="calculator.hours" />
        </div>
        <div v-if="calcMode === 'stunde'">
          <label class="block text-sm font-medium mb-1">{{ $t('services.hourlyRateLabel') }}</label>
          <input type="number" min="0" class="w-full border rounded px-3 py-2" v-model.number="calculator.userRate" />
        </div>
        <div v-if="calcMode === 'pauschal'">
           <label class="block text-sm font-medium mb-1">{{ $t('services.flatLabel') || 'Pauschalbetrag (€)' }}</label>
          <input type="number" min="0" class="w-full border rounded px-3 py-2" v-model.number="calculator.flat" />
        ...
        <div>
          ...
          <input type="number" min="0" class="w-full border rounded px-3 py-2" v-model.number="calculator.distanceKm" />
           <div class="text-xs text-gray-500 mt-1">{{ $t('services.distanceNote', { rate: distanceRate.toFixed(2) }) }}</div>
           <div class="text-xs text-gray-400">{{ $t('services.chargeableKmLabel') }} {{ chargeableKm }}</div>
        </div>
      ...
      <div class="mt-6">
        ...
        <div class="grid grid-cols-1 sm:grid-cols-3 gap-4">
          <div>
            ...
            <input type="number" min="0" class="w-full border rounded px-3 py-2" v-model.number="calcConfig.babPercent" />
          </div>
          <div>
            <label class="block text-sm mb-1">{{ $t('services.settingsVat') }}</label>
            <input type="number" min="0" class="w-full border rounded px-3 py-2" v-model.number="calcConfig.vatPercent" />
          </div>
          <div>
            <label class="block text-sm mb-1">{{ $t('services.settingsDistance') }}</label>
            <input type="number" min="0" step="0.01" class="w-full border rounded px-3 py-2" v-model.number="calcConfig.distanceRate" />
          </div>
        </div>
      </div>
      ...
      <div class="mt-6 grid grid-cols-1 sm:grid-cols-2 gap-4 text-sm">
        <div class="space-y-1">
          <div class="flex justify-between"><span>{{ $t('services.summary.baseNet') }}</span><span>{{ baseNet.toFixed(2) }} €</span></div>
          <div class="flex justify-between"><span>{{ $t('services.summary.babAmount', { percent: calcConfig.babPercent }) }}</span><span>{{ babAmount.toFixed(2) }} €</span></div>
          <div class="flex justify-between font-medium"><span>{{ $t('services.summary.subtotal') }}</span><span>{{ netPlusBab.toFixed(2) }} €</span></div>
        </div>
        <div class="space-y-1">
          <div class="flex justify-between"><span>{{ $t('services.summary.vat', { percent: calcConfig.vatPercent }) }}</span><span>{{ vatAmount.toFixed(2) }} €</span></div>
          <div class="flex justify-between font-semibold text-blue-700"><span>{{ $t('services.summary.totalGross') }}</span><span>{{ totalGross.toFixed(2) }} €</span></div>
        </div>
      </div>
    </section>
  </div>
...
const zusatzsKosten = computed(() => Math.round(((berechnung.krankenkasse + berechnung.kZusatz + berechnung.pflege + berechnung.arbeitslos + berechnung.soli + berechnung.rente + berechnung.kkinder) * 2) / 100 * berechnung.brutto))
const stundensatz = computed(() => Math.round((berechnung.brutto + berechnung.vl + zusatzsKosten.value) / 156) * berechnung.gewinn / 10)
const lizenzenSum = computed(() => Object.values(berechnung.material).reduce((a, b) => a + Math.round(b), 0));
...
const leistungen = reactive([
  ...
  },
  {
    ...
    beschreibung:
      'Erstellung einer Erweiterung für ein CMS (DLE).',
    pauschal: 350,
    stunde: stundensatz,
    pauschalOnly: false,
    minHours: 20,
  },
  {
    ...
    beschreibung:
      'Erstellung einer Webanwendung in PHP, Python oder Java',
    pauschal: 1500,
    stunde: stundensatz,
    pauschalOnly: false,
    minHours: 100
  },
  {
    ...
    beschreibung:
      'Erstellung einer Anwendung in C#, Python oder JAVA',
    pauschal: 1500,
    stunde: stundensatz,
    pauschalOnly: false,
    minHours: 100,
  },
  {
    ...
    beschreibung:
      'Administration der Webseite, Hosting, ... (mtl. Zusatzleistung)',
    pauschal: 100,
    stunde: stundensatz,
    pauschalOnly: true,
    minHours: 100,
